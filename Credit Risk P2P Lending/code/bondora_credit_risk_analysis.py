...
_MPLCONFIGDIR = PROJECT_ROOT / "tmp" / "matplotlib"
...
DEFAULT_CSV_PATH = str(PROJECT_ROOT / "data" / "LoanData.csv")
COUNTRIES = ["EE", "FI", "ES"]
...
def load_closed_loans() -> pd.DataFrame:
    if not CLOSED_LOANS_PATH.exists():
        sys.exit(f"[error] not found: {CLOSED_LOANS_PATH}\n"
                 f"        Run code/bondora_credit_risk_analysis.py pipeline first.\n")
...
def pct_ticks(ax, step: float = 0.2, top: float = 1.0) -> None:
    ticks = np.arange(0, top + 1e-9, step)
    ax.set_yticks(ticks)
    ax.set_yticklabels([f"{int(p * 100)}%" for p in ticks])
...
def plot_country_default_rates(closed: pd.DataFrame) -> pd.DataFrame:
    ...
    # Bar labels: values only, no method commentary inside the marks.
    ...
    ax.set_ylabel("Default rate\nofficial default label (L3)")
    ax.set_ylim(0, bracket_y + 0.12)
    pct_ticks(ax, step=0.2, top=1.0)
    ax.grid(False, axis="x")
    ax.set_title("Default rate by market",
                 pad=14, loc="left")
...
# Figure 2 : default-label sensitivity
...
def plot_default_label_sensitivity(closed: pd.DataFrame) -> pd.DataFrame:
    ...
    order = [k for k in ["L1", "L2", "L3", "L4", "L5"] if k in rates]
    y_pos = np.arange(len(order))[::-1]
    defaults = np.array([rates[k] for k in order])
    label_text = {
        "L1": "L1 strict late",
        "L2": "L2 status default",
        "L3": "L3 official",
        "L4": "L4 ever 60d+",
        "L5": "L5 cure-aware",
    }
    ...
    colors = [COUNTRY_COLORS["EE"] if k == "L3" else DEFAULT_FILL for k in order]
    bars = ax.barh(y_pos, defaults, color=colors, edgecolor="#222",
                   linewidth=0.7, height=0.62)

    ax.set_yticks(y_pos)
    ax.set_yticklabels([label_text.get(k, k) for k in order])
    for bar, r in zip(bars, defaults):
        ax.text(r + 0.018, bar.get_y() + bar.get_height() / 2,
                fmt_pct(r, 1), ha="left", va="center",
                fontsize=12, fontweight="bold")

    ax.annotate(
        f"{spread_pp:.1f}pp\nspread",
        xy=(max(defaults), y_pos[int(defaults.argmax())]),
        xytext=(0.58, y_pos[int(defaults.argmax())] + 0.55),
        ha="left",
        va="center",
        fontsize=10.5,
        color=TEXT_MUTED,
    ...
    ax.set_xlim(0, max(defaults) + 0.18)
    ax.set_xticks(np.arange(0, 0.81, 0.2))
    ax.set_xticklabels([f"{int(p * 100)}%" for p in np.arange(0, 0.81, 0.2)])
    ax.set_xlabel("Default share")
    ax.set_title("Label choice shifts base rate",
                 pad=12, loc="left")
    ax.grid(False, axis="y")
    ax.spines["left"].set_visible(False)

    fig.text(0.5, -0.03,
             f"L3 L4 overlap {jaccard:.3f}",
             ha="center", fontsize=10.5, style="italic", color=TEXT_MUTED)
...
def _plot_calibration_panel(
    ...
    metric_ax.legend(handles, labels, loc="upper left",
...
def plot_pod_calibration(preds: pd.DataFrame) -> pd.DataFrame:
    ...
    all_rows = []
    horizons = [
        ("1-year calibration", y1_col, horizon_cols["1y"],
         "pod_calibration_1year"),
        ("Lifetime calibration (L3)", ylife_col, horizon_cols["lifetime"],
...
def _draw_brier_components_bar(ax, decomp: Dict[str, float], title: str,
    ...
    for x, val, color, txt, label in items:
        ...
        y_text = val + ymax * 0.018 if val >= 0 else val - ymax * 0.018
        va = "bottom" if val >= 0 else "top"
        ax.text(x, y_text, txt, ha="center", va=va,
...
def plot_brier_score_decomposition(preds: pd.DataFrame) -> pd.DataFrame:
    ...
    axes[0].set_ylabel("Contribution to Brier score")
...
def plot_entry_vintage_default_rates(closed: pd.DataFrame) -> pd.DataFrame:
    ...
    ax.axhline(ee_entry, color=COUNTRY_COLORS["EE"], linestyle=":",
               ...
               label=f"EE baseline {ee_entry * 100:.1f}%")
