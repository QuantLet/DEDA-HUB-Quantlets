...
def read_csv_quiet(path: Path | str) -> pd.DataFrame:
    ...
    if not raw.empty:
        ...
        try:
            ...
            if err.name != "matplotlib":
                raise
            print("matplotlib not installed; skipped four-score plots.")
...
def apply_style():
    ...
    matplotlib.rcParams.update({
        ...
        "font.size": 15, "axes.titlesize": 16, "axes.titleweight": "bold",
...
def target_line(ax, y=0.90, label="target"):
    ax.axhline(y, ls="--", lw=1.4, color=TARGET, zorder=1)
    ax.text(0.015, 0.965, label, color=TARGET, fontsize=11.0, va="top",
            ha="left", transform=ax.transAxes)

def caption(fig, text, y=0.012, fontsize=11.5):
...
def fig_s6(outdir):
    ...
    ac1 = [1.253, 1.212, 1.209, 1.194]
    acL = [1.526, 1.386, 1.393, 1.486]
...
def fig_s8(outdir):
    apply_style()
    ctry = ["EE", "FI", "ES"]
    pooled = [0.9144, 0.9080, 0.8439]
    per_country = [0.9022, 0.9012, 0.9007]
    amb = [0.099, 0.102, 0.731]
    colors = [COUNTRY[c] for c in ctry]
    x = np.arange(3)

    fig, ax = plt.subplots(1, 2, figsize=(12.8, 5.8), sharey=True)
    panels = [
        (ax[0], pooled, "pooled", False),
        (ax[1], per_country, "per-country", True),
    ]

    for a, values, title, show_amb in panels:
        bars = a.bar(x, values, width=0.6, color=colors,
                     edgecolor="#333333", linewidth=0.8)
        target_line(a)
        a.set_xticks(x)
        a.set_xticklabels(ctry)
        a.set_ylim(0.80, 0.95)
        a.set_title(title)
        for i, bb in enumerate(bars):
            v = bb.get_height()
            col = STATUS["broken"] if v < 0.895 else "#333333"
            label = f"{v:.3f}"
            if show_amb:
                label += f"\namb {amb[i]:.2f}"
            a.text(bb.get_x() + bb.get_width() / 2, v + 0.005, label,
                   ha="center", va="bottom", fontsize=12.5,
                   fontweight="bold", color=col)

    ax[0].set_ylabel("coverage")
    fig.suptitle("Spain under-coverage",
                 fontsize=19, fontweight="bold", y=0.99)
    caption(fig, "coverage restored; ES amb 0.73")
    fig.tight_layout(rect=[0, 0.05, 1, 0.93])
    _save(fig, outdir, "group_conditional_coverage.png")

def fig_s9(outdir):
    apply_style()
    G = ["EE to EE", "EE to FI", "EE to ES"]; gv = [0.902, 0.892, 0.819]
    ...
    ax[0].set_ylim(0.70, 1.00); ax[0].set_title("Geographic transfer", fontsize=15)
    ...
    ax[1].set_ylim(0.70, 1.00); ax[1].set_title("Calendar check", fontsize=15)
    ...
    fig.suptitle("Market shift", fontsize=19, fontweight="bold", y=0.99)
    caption(fig, "target line")
...
def fig_s14(outdir):
    ...
    fig = plt.figure(figsize=(12.4, 7.4), constrained_layout=False)
    ax_l3 = fig.add_axes([0.08, 0.50, 0.40, 0.27])
    ax_rob = fig.add_axes([0.56, 0.50, 0.40, 0.27])
    ax_width = fig.add_axes([0.32, 0.08, 0.36, 0.22])
    x = np.arange(5)

    b = ax_l3.bar(x, l3, color=lc, width=0.62, edgecolor="white"); target_line(ax_l3)
    ax_l3.set_ylim(0.78, 1.00)
    ax_l3.set_xticks(x); ax_l3.set_xticklabels(L); ax_l3.set_ylabel("coverage by label")
    ax_l3.text(0.5, -0.20, r"$q=q_3$", transform=ax_l3.transAxes,
               ha="center", va="top", fontsize=12.5, color="#555555")
    for bb, v in zip(b, l3):
        bad = v < 0.895
        ax_l3.text(bb.get_x()+bb.get_width()/2, v+0.004, f"{v:.3f}", ha="center", va="bottom",
                   fontsize=12.5, fontweight="bold", color=(STATUS["broken"] if bad else "#333333"))

    b = ax_rob.bar(x, worst, color=SCORE["PoD"], width=0.62, edgecolor="white"); target_line(ax_rob)
    ax_rob.set_ylim(0.78, 1.00)
    ax_rob.set_xticks(x); ax_rob.set_xticklabels(L)
    for bb, v in zip(b, worst):
        ax_rob.text(bb.get_x()+bb.get_width()/2, v+0.004, f"{v:.3f}", ha="center", va="bottom", fontsize=12.5, fontweight="bold")

    wb = ax_width.bar([0, 1], [1.39, 1.63], color=[STATUS["neutral"], STATUS["broken"]], width=0.52, edgecolor="white")
    ax_width.set_ylim(1.0, 1.8)
    ax_width.set_xticks([0, 1])
    ax_width.set_xticklabels([r"$q_3$", r"$q_{\mathrm{rob}}$"])
    ax_width.set_ylabel("set width")
    ax_width.text(0.5, -0.22, r"$A(q)=P(|C_q(X)|=2)$",
               transform=ax_width.transAxes, ha="center", va="top",
               fontsize=11.5, color="#555555")
    for bb, v, m in zip(wb, [1.39, 1.63], [0.39, 0.63]):
        ax_width.text(bb.get_x()+bb.get_width()/2, v+0.012, f"{v:.2f}\namb {m:.2f}", ha="center", va="bottom", fontsize=12, fontweight="bold")

    fig.suptitle("Label robust width cost", fontsize=19, fontweight="bold", y=0.99)
    top_titles = [
        (ax_l3, "Coverage under $q_3$:\ncalibrated to L3 only"),
        (ax_rob, r"Coverage under $q_{\mathrm{rob}}=\mathrm{max}_m q_m$"),
    ]
    top_y = 0.86
    for a, title in top_titles:
        box = a.get_position()
        fig.text((box.x0 + box.x1) / 2, top_y, title,
                 ha="center", va="top", fontsize=16,
                 fontweight="bold", color="#000000", linespacing=1.1)
    box = ax_rob.get_position()
    fig.text((box.x0 + box.x1) / 2, 0.785,
             "worst-case threshold, here driven by L1",
             ha="center", va="bottom", fontsize=9.5, color="#555555")
    box = ax_width.get_position()
    fig.text((box.x0 + box.x1) / 2, 0.365,
             r"Width price: $W(q)=E|C_q(X)|$",
             ha="center", va="bottom", fontsize=16,
             fontweight="bold", color="#000000")
    _save(fig, outdir, "label_robustness_coverage.png")
...
def fig_s17(outdir):
    ...
    for a, cov, t in [(ax[0, 0], cov1, "1-year   (n = 76,369, base 0.187)"),
        ...
        a.set_ylim(0.62, 1.06); a.set_title(t); a.set_xticks(x); a.set_xticklabels(lab)
        for bb, c in zip(b, cov):
    ...
    ax[1, 0].set_ylabel("set width")
    fig.suptitle("TabPFN conformal set", fontsize=19, fontweight="bold", y=0.985)
    caption(fig, "Valid set.")
...
def fig_s11(master, outdir, alpha=0.1, warmup=8000, window=2000, roll=2000,
    ...
    apply_style()
    mp = Path(master)
    df = pd.read_parquet(mp) if mp.suffix == ".parquet" else read_csv_quiet(mp)
    ...
    e = e.dropna(subset=[dc, sc, yc]).sort_values(dc).reset_index(drop=True)
    ...
    target_line(ax[0], label="target 0.90")
    ax[0].set_ylim(0.70, 1.00)
    ...
    ax[1].set_ylabel("rolling avg |C|")
    ax[1].set_xlabel("loan stream, ordered by origination date   (grey = high FI/ES-share epoch)")
    caption(
        fig,
        "plain drifts below 0.90 where the market moves; ACI / PID track back to 0.90 (curves coincide). Narrow-and-wrong vs wide-and-honest.",
        y=0.008,
        fontsize=11.5,
    )
    fig.tight_layout(rect=[0, 0.05, 1, 0.98]); _save(fig, outdir, "online_adaptation_coverage.png")
...
_Q_MARK = [0.10, 0.20]; _N_SEEDS = 5; _SEED0 = 0
...
def _s16_horizon(df, split_col, horizon, cols):
    ...
    pool = heldout_frame(df, horizon, p1c)
    p1 = pool[p1c].to_numpy(float); y = pool[target_col(horizon)].to_numpy(int)
    n = len(pool); cal_size = original_cal_size(pool); base = float(y.mean())
    acc = {q: {"fdr": [], "power": []} for q in _Q_GRID}
    for seed in range(_N_SEEDS):
        ci, ti = random_cal_eval_indices(n, cal_size, _SEED0 + seed)
...
def fig_s16(master, outdir, preds=None):
    ...
    apply_style()
    mp = Path(master)
    df = pd.read_parquet(mp) if mp.suffix in (".parquet", ".pq") else read_csv_quiet(mp)
    if preds and Path(preds).exists():
        pr = read_csv_quiet(preds)
    ...
    ax[0].text(qmax, qmax, r" realized FDR=$q_{\mathrm{FDR}}$",
    ...
    ax[0].text(0.255, 0.205, "baseline bad-loan rate",
               fontsize=8.8, color="#555555", ha="left", va="center")
    ax[0].set_xlabel(r"target FDR $q_{\mathrm{FDR}}$"); ax[0].set_ylabel("realized FDR")
    ax[0].set_title("FDR controlled", fontsize=15)
    ax[0].legend(loc="upper left", frameon=False)
    ax[1].set_xlabel(r"target FDR $q_{\mathrm{FDR}}$"); ax[1].set_ylabel("approval power")
    ax[1].set_title("Approval gain", fontsize=15)
    ax[1].legend(loc="upper left", frameon=False)
    fig.suptitle("FDR-controlled approval", fontsize=19, fontweight="bold", y=0.99)
    caption(fig, r"$q_{\mathrm{FDR}}$ is the risk-appetite dial; circles mark $q_{\mathrm{FDR}}$ 0.10 and 0.20.")
